...
{
   Pmsg4(50, "\t\t>>>> %4d as_acquire_buffer() parent: %p, workq->quit: %d, free buffers: %d\n",
      my_thread_id(), parent, as_workqueue_engine_quit(), qsize(&as_free_buffer_queue));
   ...
   while (1)
   {
      ...
      if (buffer)
      {
         ...
         Pmsg4(50, "\t\t>>>> %4d as_acquire_buffer() parent: %p, buffer: %d, free buffers: %d\n",
            my_thread_id(), parent, buffer->id, qsize(&as_free_buffer_queue));
         break;
      }
   }
...
void as_consumer_enqueue_buffer(as_buffer_t *buffer, bool finalize)
{
	...
	if (as_bigfile_bsock_proxy == NULL)
	{
	   ...
	   Pmsg3(50, "\t\t>>>> %4d as_consumer_enqueue_buffer() %d, NEW bigfile: %X\n",
         my_thread_id(), buffer->id, as_bigfile_bsock_proxy);
	...
	else if (as_bigfile_bsock_proxy == buffer->parent)
	{
      if (finalize)
      {
         Pmsg3(50, "\t\t>>>> %4d as_consumer_enqueue_buffer() %d, LAST bigfile: %X\n",
            my_thread_id(), buffer->id, as_bigfile_bsock_proxy);
      }
      else
      {
         Pmsg3(50, "\t\t>>>> %4d as_consumer_enqueue_buffer() %d, CONT bigfile: %X\n",
            my_thread_id(), buffer->id, as_bigfile_bsock_proxy);
      }
	...
	{
      Pmsg3(50, "\t\t>>>> %4d as_consumer_enqueue_buffer() %d, BLOCKED bigfile: %X\n",
         my_thread_id(), buffer->id, as_bigfile_bsock_proxy);
      QINSERT(&as_consumer_buffer_queue, &buffer->bq);
	}
...
as_buffer_t *as_consumer_dequeue_buffer()
{
   ...
   if (buffer)
   {
      Pmsg3(50, "\t\t>>>> %4d as_consumer_dequeue_buffer() %d, consumer queue size: %d\n",
...
void as_release_buffer(as_buffer_t *buffer)
{
   ...
   Pmsg3(50, "\t\t>>>> %4d as_release_buffer() %d, consumer queue size: %d\n",
...
void *as_consumer_thread_loop(void *arg)
{
   ...
   while (as_quit_consumer_thread_loop() == false)
   {
      ...
      Pmsg6(50, "\t\t>>>> %4d as_consumer_thread_loop() buf: %d, tosend: %d, pos: %d, bufsize: %d, qsize: %d\n",
...
void as_init_consumer_thread(BSOCK *sd)
{
   ...
   Pmsg3(50, "\t\t>>>> %4d as_init_consumer_thread() id: %X sock: %p\n",
      my_thread_id(), as_consumer_thread, sd);
}
...
void as_release_remaining_consumer_buffers()
{
   ...
   Pmsg1(50, "\t\t>>>> %4d as_release_remaining_consumer_buffers() START\n", my_thread_id());
	...
	do
	{
		...
		if (buffer != NULL)
		{
		   Pmsg2(50, "\t\t>>>> %4d as_release_remaining_consumer_buffers() buffer: %d\n", my_thread_id(), buffer->id);
			QINSERT(&as_free_buffer_queue, &buffer->bq);
		}
	} while (buffer != NULL);

	Pmsg3(50, "\t\t>>>> %4d as_release_remaining_consumer_buffers() END, consumer queue size %d, free queue size %d\n",
...
{
   Pmsg2(50, "\t\t>>>> %4d as_clear_free_buffers_queue() BEGIN, size %d\n",
   ...
   Pmsg2(50, "\t\t>>>> %4d as_clear_free_buffers_queue() END, size %d\n",
      my_thread_id(), qsize(&as_free_buffer_queue));
